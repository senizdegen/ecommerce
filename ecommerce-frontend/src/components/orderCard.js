const statusConfig = {
  ...
  PAID:      { color: 'bg-blue-50 text-blue-700 border-blue-200',           dot: 'bg-blue-400'    },
  SHIPPED:   { color: 'bg-indigo-50 text-indigo-700 border-indigo-200',     dot: 'bg-indigo-400'  },
...
export function orderCard(order) {
  ...
  const cfg = statusConfig[status] || { color: 'bg-gray-100 text-gray-700 border-gray-200', dot: 'bg-gray-400' };
        ...
        return `
          <div class="flex items-center ${i < steps.length - 1 ? 'flex-1' : ''}">
            <div class="flex flex-col items-center gap-1">
              ...
              <span class="text-xs ${active ? 'text-red-500 font-semibold' : done ? 'text-gray-600' : 'text-gray-300'} whitespace-nowrap">
                ${stepLabels[step]}
              </span>
            </div>
      ...
      ${order.items.map(item => `
        <div class="flex items-center gap-4 py-3 border-b border-gray-50 last:border-0">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-800 truncate">${item.name || item.productUid}</p>
      ...
      <button onclick="document.getElementById('${cardId}').classList.toggle('hidden')" class="w-full text-left">
        <div class="px-6 py-5 flex items-center gap-4">
          ...
          <div class="flex items-center gap-4 flex-shrink-0">
            <span class="text-base font-bold text-gray-900">$${amount.toFixed(2)}</span>
